...
if __name__=="__main__":
    ...
    parser.add_argument("--reset_p", type=float, default=1.0)
    ...
    save_dir = f"{args.save_dir}/{args.dataset}/CLIP@resetP{args.reset_p}_delta{args.delta}_lambda{args.lambda_sam}_alpha_{args.lora_alpha}_LoRA_{args.arch.replace('/', '')}"
    ...
    _, trainable_params = loralib.get_lora_params(model, fc=True, idxs=lora_idxs, train_visual_proj=False, train_text_proj=False, train_text_encoder=False)
    ...
    # if args.lr_schedule: scheduler = cosine_lr(optimizer, args.lr, args.warmup, 1*num_batches)
    ...
    for epoch in range(1, args.epochs+1):
        model.train()      
        pbar = tqdm(train_loader, desc=f'Epoch: {epoch:03d}', ncols=150)

        if epoch > 1:
            for name, module in model.named_modules():
                if isinstance(module, loralib.LoRAInjectedLinear):
                    module.lora0_A.weight.data.mul_(args.reset_p)
                    module.lora0_B.weight.data.mul_(args.reset_p)
                    current_update = torch.matmul(module.lora0_B.weight, module.lora0_A.weight).reshape(-1)
                    magnitude = current_update.norm(p=2)
                    wandb.log({f"lora_magnitude/{name}": magnitude.item()})
                    ...
                    for subname, submodule in module.named_modules():
                        if isinstance(submodule, loralib.LoRAInjectedLinear):
                            ...
                            submodule.lora0_A.weight.data.mul_(args.reset_p)
                            submodule.lora0_B.weight.data.mul_(args.reset_p)
                            current_update = torch.matmul(submodule.lora0_B.weight, submodule.lora0_A.weight).reshape(-1)
                            magnitude = current_update.norm(p=2)
                            wandb.log({f"lora_magnitude/{full_name}": magnitude.item()})
        ...
        for data_i, data in enumerate(pbar):
            ...
            #if args.lr_schedule: scheduler(data_i)
            ...
            model.module.set_lora_scaling(args.r / args.lora_alpha)
            ...
            image_features, text_features, logit_scale = model(images, clip.tokenize(class_prompts).to("cuda"), False)
            ...
            if args.lambda_sam > 0:
                ### === α + δ (perturbed) forward === ###
                model.module.set_lora_scaling((args.r / args.lora_alpha) * (1+args.delta))

                image_features_pert, text_features_pert, logit_scale_pert = model(images, clip.tokenize(class_prompts).to("cuda"), False)

                logit_scale_pert = logit_scale_pert if len(list(range(torch.cuda.device_count()))) == 1 else logit_scale_pert[0]
                logits_per_image_pert = logit_scale_pert * image_features_pert @ text_features_pert.t()
                logits_per_text_pert = logits_per_image_pert.t()
                cl_loss_pert = (F.cross_entropy(logits_per_image_pert, labels) + F.cross_entropy(logits_per_text_pert, labels)) / 2

                ### === α-descent reward loss === ###
                reward_term = torch.clamp(cl_loss_pert - cl_loss, min=0.0)
                loss = cl_loss + args.lambda_sam * reward_term
            else:
                cl_loss_pert = torch.tensor(0.).to("cuda")
                loss = cl_loss
            ...
            wandb.log({
                ...
                "step/loss_pert": cl_loss_pert.item(),
        ...
        with torch.no_grad():
            ...
            for scale in [0.5, 1.0]:
                ...
                model.module.set_lora_scaling(scale * (args.r / args.lora_alpha))
                class_embs = []
                ...
                # with torch.no_grad():
                #     worst_acc, avg_acc, accs_by_group = evaluate(*infer(model, train_loader, None, desc=f"Eval Train", class_embs=class_embs), str=False)
                # print(f"Epoch {epoch}) Train Set - accuracy: {avg_acc:.2f} \n")
                # f.write(f"Train Set - accuracy: {avg_acc:.2f} \n")
                ...
                if worst_acc >= best_val_worst_acc:
                    ...
                    for test_dataset, test_loader, projection_fn in zip(test_datasets, test_loaders, projection_fns):
                        with torch.no_grad():
                            ...
                            print(f"Epoch {epoch}) Test Set ({test_dataset}) - accuracy: {test_avg_acc:.2f} \n")
                        f.write(f"Test Set ({test_dataset}) - accuracy: {test_avg_acc:.2f} \n")
            model.module.set_lora_scaling(args.r / args.lora_alpha)
    f.write('\n')
    f.close()
    wandb.finish()