...
if __name__=="__main__":
    ...
    parser.add_argument("--lambda_scale", type=float, default=1.)
    ...
    save_dir = f"{args.save_dir}/{args.dataset}/CLIP@backward_{args.lambda_scale}_LoRA_{args.arch.replace('/', '')}"
    ...
    if args.lambda_feat_ortho > 0. :
        ...
        loralib.apply_lora(model, num_lora, args.r, args.lora_alpha, args.lora_dropout, lora_modules, ortho_feat_loss_fn, gating=args.gating, lora_intermediate=args.lora_intermediate, visual_only=False, lambda_scale=args.lambda_scale)
    else:
        ...
        loralib.apply_lora(model, num_lora, args.r, args.lora_alpha, args.lora_dropout, lora_modules, gating=args.gating, lora_intermediate=args.lora_intermediate, visual_only=False, lambda_scale=args.lambda_scale)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for data_i, data in enumerate(pbar):
            ...
            if args.lambda_feat_ortho > 0.: 
                ...
                ortho_loss += feat_loss

            loss = cl_loss + args.lambda_feat_ortho * ortho_loss 
            loss.backward()
