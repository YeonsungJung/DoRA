...
if __name__=="__main__":
    ...
    parser.add_argument("--m_start", type=float, default=1.0)
            ...
            for name, module in model.named_modules():
                if isinstance(module, loralib.LoRAInjectedLinear):
                    if hasattr(module, 'lora0_A') and hasattr(module, 'lora0_B'):
                        baseline_update = torch.matmul(module.lora0_B.weight.data, module.lora0_A.weight.data).reshape(-1)
                        norm_val = baseline_update.norm(p=2) + 1e-8
                        v = baseline_update / norm_val
                        projection_dirs[name] = v.clone()
                    ...
                    for subname, submodule in module.named_modules():
                        if isinstance(submodule, loralib.LoRAInjectedLinear):
                            if hasattr(submodule, 'lora0_A') and hasattr(submodule, 'lora0_B'):
                                ...
                                norm_val = baseline_update.norm(p=2) + 1e-8
                                v = baseline_update / norm_val
                                projection_dirs[full_name] = v.clone()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for data_i, data in enumerate(pbar):
            ...
            proj_reg_loss = torch.tensor(0.).to("cuda")
            cnt = 0
            layerwise_logs = {}
            if projection_dirs: # and data_i >500
                for name, module in model.named_modules():
                    if isinstance(module, loralib.LoRAInjectedLinear):
                        ...
                        for subname, submodule in module.named_modules():
                            if isinstance(submodule, loralib.LoRAInjectedLinear):
                                ...
                                layerwise_logs[f"norm/{full_name}/proj_reg_loss"] = proj_component.norm(p=2).item()

                            
            loss = cl_loss + args.lambda_reg * (proj_reg_loss/cnt)
            ...
            layerwise_logs.update({
                ...
                "step/loss_projreg": args.lambda_reg * (proj_reg_loss/cnt).item(),
            ...
            pbar.set_postfix({
                ...
                "reg_loss": (args.lambda_reg * proj_reg_loss).item()
            })
