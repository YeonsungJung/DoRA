...
if __name__=="__main__":
    ...
    if args.model == "CLIP":
        ...
        model = clip.CLIP_FT_desc(args, "cuda", train_dataset.class_descs, freeze_encoder=True)
    else:
    ...
    if args.text_cls or args.desc_cls:
        ...
        for k in train_dataset.classnames:
            if args.desc_cls:
                all_descs = [[template.format(desc) for template in imagenet_templates] for desc in train_dataset.class_descs[k]]
                all_descs = [model.model.encode_text(clip.tokenize(desc).to("cuda")) for desc in all_descs]
                all_descs = [desc / desc.norm(dim=-1, keepdim=True) for desc in all_descs]
                all_descs = [desc.mean(dim=0) for desc in all_descs]
                all_descs = [desc / desc.norm() for desc in all_descs]
                class_emb = torch.stack(all_descs, dim=0).detach()
                class_emb = class_emb.mean(dim=0)
                class_emb = class_emb / class_emb.norm()

                if args.desc_lambda != 0:       
                    tmp = [template.format(k) for template in imagenet_templates]
                    tmp = model.model.encode_text(clip.tokenize(tmp).to("cuda"))
                    tmp = tmp / tmp.norm(dim=-1, keepdim=True)
                    tmp = tmp.mean(dim=0)
                    tmp = tmp / tmp.norm()

                    class_emb = args.desc_lambda * class_emb + (1-args.desc_lambda) * tmp
    ...
    if args.cl:
        ...
        delattr(model.model, 'token_embedding')
        delattr(model.model, 'positional_embedding')
        delattr(model.model, 'ln_final')
        delattr(model.model, 'text_projection')
        delattr(model.model, 'logit_scale')
    ...
    if args.lambda_feat_ortho > 0. :
        loralib.apply_lora(model, num_lora, args.r, args.lora_alpha, args.lora_dropout, lora_modules, ortho_feat_loss_fn, args.ortho_pretrained, gating=args.gating, lora_intermediate=args.lora_intermediate)
    else:
    ...
    _, trainable_params = loralib.get_lora_params(model, fc=True, idxs=lora_idxs, train_visual_proj=args.train_visual_proj, train_text_encoder= not args.freeze_text, gating=args.gating)
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for data_i, data in enumerate(tqdm(train_loader, desc=f'Epoch: {epoch:03d}', ncols=100)):
            ...
            if args.cl:
                ...
                image_features, text_features, logit_scale, desc_loss, feat_loss = model(images, clip.tokenize(class_prompts).to("cuda"))
                ...
                cls_loss = (F.cross_entropy(logits_per_image, labels) + F.cross_entropy(logits_per_text, labels)) / 2
                ...
                if args.text_cls or args.desc_cls:
                    cls_loss = cls_loss_fn((outputs/outputs.norm(dim=-1, keepdim=True)) @ class_embs.T, labels)
                else:
            ...
            #train_worst_acc, train_avg_acc, _ = evaluate(preds.detach().cpu().numpy(), labels.cpu().numpy(), spurious.numpy())
            ...
            wandb.log({
                ...
                "step/loss_ortho": ortho_loss.item() / args.lambda_feat_ortho if args.lambda_feat_ortho != 0 else 0,
        ...
        loralib.save_lora(model.module, latest_model_path, idxs=lora_idxs, train_visual_proj=args.train_visual_proj, train_text_encoder=args.cl)
