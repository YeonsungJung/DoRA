...
def save_lora_grad(name, grad_dict):
    def hook(module, grad_input, grad_output):
        grad = module.lora0_B.weight.grad @ module.lora0_A.weight  # shape: [out_dim, in_dim]
        grad_dict[name].append(grad.detach().reshape(-1))
    return hook
...
if __name__=="__main__":
    ...
    # projection reg.
    parser.add_argument("--lambda_reg_dir", type=float, default=0.1)
    parser.add_argument("--lambda_reg_norm", type=float, default=0.1)
    ...
    save_dir = f"{args.save_dir}/{args.dataset}/CLIP@projReg_{args.lambda_reg_dir}_{args.lambda_reg_norm}_LoRA_{args.arch.replace('/', '')}"
    ...
    #             module.scaling = 1 / module.r
    #     elif isinstance(module, loralib.LoRAInjectedMultiheadAttention):
    #         for submodule in module.modules():
    #             if isinstance(submodule, loralib.LoRAInjectedLinear):
    #                 if hasattr(submodule, 'lora0_A') and hasattr(submodule, 'lora0_B'):
    #                     submodule.init_lora_params()
    ...
    # svd
    grad_buffer = defaultdict(list)
    MAX_SVD_STEPS = 200

    for name, module in model.named_modules():
        if isinstance(module, loralib.LoRAInjectedLinear):
            module.register_full_backward_hook(save_lora_grad(name, grad_buffer))
        elif isinstance(module, loralib.LoRAInjectedMultiheadAttention):
            for subname, submodule in module.named_modules():
                if isinstance(submodule, loralib.LoRAInjectedLinear):
                    full_name = f"{name}.{subname}"
                    submodule.register_full_backward_hook(save_lora_grad(full_name, grad_buffer))

    

    total_steps = num_batches * args.epochs
    iteration = (1-1) * len(train_loader)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for data_i, data in enumerate(pbar):
            ...
            dir_reg_loss = torch.tensor(0.).to("cuda")
            norm_reg_loss = torch.tensor(0.).to("cuda")
            cnt = 0
            layerwise_logs = {}
            if projection_dirs:
                for name, module in model.named_modules():
                    if isinstance(module, loralib.LoRAInjectedLinear):
                        ...
                        layerwise_logs[f"norm/{name}/update_norm"] = current_update.norm(p=2).item()
            ...
            loss = cl_loss - args.lambda_reg_dir * dir_reg_loss + args.lambda_reg_norm * norm_reg_loss
            ...
            layerwise_logs.update({
                ...
                "step/reg_dir": dir_reg_loss.item(),
                "step/reg_norm": norm_reg_loss.item(),
            ...
            pbar.set_postfix({
                ...
                "reg_dir": dir_reg_loss.item(),
                "reg_norm": norm_reg_loss.item()
            })
            

            # if projection_dirs and iteration % 1000 == 0: # and data_i >500
            #     if iteration < total_steps * args.m_warmUp:
            #         m = ((args.m_end - args.m_start) / (total_steps * args.m_warmUp)) * iteration + args.m_start
            #     else:
            #         m = args.m_end
            ...
            #         for name, module in model.named_modules():
            #             if isinstance(module, loralib.LoRAInjectedLinear):
            #                 v = projection_dirs[name].detach()
            #                 current_update = torch.matmul(module.lora0_B.weight, module.lora0_A.weight).reshape(-1)
            #                 current_update_norm = current_update / (current_update.norm(p=2) + 1e-8)
            #                 v_new = m * v + (1-m) * current_update_norm.data
            #                 projection_dirs[name] = v_new / (v_new.norm(p=2) + 1e-8)

            #             elif isinstance(module, loralib.LoRAInjectedMultiheadAttention):
            #                 for subname, submodule in module.named_modules():
            #                     if isinstance(submodule, loralib.LoRAInjectedLinear):
            #                         full_name = f"{name}.{subname}"
            #                         v = projection_dirs[full_name].detach()
            #                         current_update = torch.matmul(submodule.lora0_B.weight, submodule.lora0_A.weight).reshape(-1)
            #                         current_update_norm = current_update / (current_update.norm(p=2) + 1e-8)
            #                         v_new = m * v + (1-m) * current_update_norm.data
