...
if __name__=="__main__":
    ...
    save_dir = f"{args.save_dir}/{args.dataset}/CLIP@LoRA_{args.arch.replace('/', '')}_desc"
    ...
    desc_embs = {k: v.mean(dim=0) for k,v in model.module.desc_embs.items()}
    desc_embs = torch.stack([desc_embs[k] for k in train_dataset.classnames], dim=0)
    desc_embs = desc_embs.cuda().detach()
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        for data_i, data in enumerate(tqdm(train_loader, desc=f'Epoch: {epoch:03d}', ncols=100)):
            ...
            logits_per_image_desc = logit_scale * image_features @ desc_embs.t()
            logits_per_text_desc = logits_per_image_desc.t()
            labels = data['labels'].to("cuda")
            desc_loss = F.cross_entropy(logits_per_image_desc, labels)

            # loss = cl_loss + args.pcaOrtho_lambda * pca_ortho_loss
            # loss = cl_loss
            loss = cl_loss + desc_loss
            # if epoch > 5:
            #     loss = cl_loss + args.pcaOrtho_lambda * pca_ortho_loss
            # else:
            #     loss = cl_loss
