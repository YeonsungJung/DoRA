...
if __name__=="__main__":
    ...
    save_dir = f"{args.save_dir}/{args.dataset}/CLIP@projReg_{args.lambda_reg_dir}_{args.lambda_reg_norm}_alpha_{args.lora_alpha}_LoRA_{args.arch.replace('/', '')}"
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for data_i, data in enumerate(pbar):
            ...
            l2_reg_loss = torch.tensor(0.).to("cuda")
            cnt = 0
            layerwise_logs = {}
            if args.lambda_reg_l2 > 0:
            ...
            loss = cl_loss + args.lambda_reg_l2 * l2_reg_loss
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for scale in [0.5, 1.0, 1.05, 1.1]:
                ...
                print(f"Epoch {epoch}) Train Set - Average accuracy: {avg_acc:.2f} | Worst Group accuracy: {worst_acc:.2f} | Acc by group: {[round(acc, 2) for acc in accs_by_group]}\n")
                f.write(f"Train Set - Average accuracy: {avg_acc:.2f} | Worst Group accuracy: {worst_acc:.2f} | Acc by group: {[round(acc, 2) for acc in accs_by_group]}\n")
